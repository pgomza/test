...
import com.horeca.site.services.SpaOrderService;
...
public class SpaOrderController {
    ...
    public SpaOrderView get(@PathVariable String pin, HttpServletRequest request) {
        String preferredLanguage = request.getLocale().getLanguage();
        return service.getActiveView(pin, preferredLanguage);
    }
    ...
    @RequestMapping(value = "/{pin}/orders/spa/status", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE)
    public OrderStatusPUT getStatus(@PathVariable String pin) {
        return service.getActiveStatus(pin);
    }

    @RequestMapping(value = "/{pin}/orders/spa/status", method = RequestMethod.PUT, produces = MediaType.APPLICATION_JSON_VALUE)
